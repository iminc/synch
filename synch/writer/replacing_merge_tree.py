    ...
    def get_table_create_sql(
        ...
        **kwargs,
        ...
        if ttl:
            ttl_str = f" TTL {ttl} "
        if version_column:
            return f"CREATE TABLE {schema}.{table}{cluster_sql(self.cluster_name)} ENGINE = {self.engine}({version_column}) {partition_by_str} ORDER BY {pk} {ttl_str} {engine_settings_str} AS {select_sql} limit 0"
        else:
            return f"CREATE TABLE {schema}.{table}{cluster_sql(self.cluster_name)} ENGINE = {self.engine} {partition_by_str} ORDER BY {pk} {ttl_str} {engine_settings_str} AS {select_sql} limit 0"
