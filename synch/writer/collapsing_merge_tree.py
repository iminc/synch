...
class ClickHouseCollapsingMergeTree(ClickHouseMergeTree):
    ...
    def get_table_create_sql(
        ...
        if ttl:
            ttl_str = f" TTL {ttl} "
        return f"CREATE TABLE {schema}.{table}{cluster_sql(self.cluster_name)}ENGINE = {self.engine}({sign_column}) {partition_by_str} ORDER BY {pk} {ttl_str} {engine_settings_str} AS {select_sql} limit 0"
